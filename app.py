...
if not oscilaciones_disponibles or len(oscilaciones_disponibles) < 10:
    st.info("Inicializando datos climáticos... Verificando y descargando las series de índices climáticos. Este proceso puede tardar unos minutos.")
...
year_op_default, mes_op_default = determinar_ultimo_mes_disponible(oscilaciones_disponibles)

# ==============================================================================
# PANEL LATERAL DE NAVEGACIÓN Y PARÁMETROS
# ==============================================================================
st.sidebar.image("https://raw.githubusercontent.com/wabarca/aanalogos/master/docs/img/marn_logo.png" if False else None, width=180)
...
if seccion_seleccionada == "🌦️ Análisis de Años Análogos":
    ...
    indices_seleccionados_str = st.sidebar.multiselect(
        ...
        default=[NOMBRES_LEGIBLES[k] for k in ["PDO", "TNA", "ONI"] if k in NOMBRES_LEGIBLES],
    ...
    year_op_calc, mes_op_calc = determinar_ultimo_mes_disponible(oscilaciones_disponibles, indices=indices_codigos)
    ...
    if modo_analisis == "Operacional":
        ...
        st.sidebar.markdown(f"**Mes Objetivo:** `{mes_objetivo} — {NOMBRES_MESES[mes_objetivo - 1]}` *(Último utilizable)*")
        ...
        tipo_reanalisis = st.sidebar.radio(
            ...
            ["Retrospectivo Completo (Todos los años históricos)", "Backtesting / Simulación Tiempo Real (Corte en $Y_{\\text{obj}}$)"],
            index=0,
    ...
    with col_info:
        if modo_analisis == "Operacional":
            desc_v_op = obtener_descripcion_ventana(year_op_calc, mes_op_calc, longitud_ventana=longitud_ventana)
    ...
    if resultado is None:
        st.warning("⚠️ Debe seleccionar al menos un índice climático para realizar el cálculo.")
        ...
        if resultado.indices_no_disponibles:
            st.markdown(f"**Índices no disponibles para este período:** `{', '.join(resultado.indices_no_disponibles)}`")
    else:
        # Métricas principales (KPIs)
        st.subheader("📌 Resumen del Análisis Climatológico")
        kpi1, kpi2, kpi3, kpi4, kpi5 = st.columns(5)
        kpi1.metric("Año Objetivo ($Y_{\text{obj}}$)", f"{resultado.year_objetivo}")
        kpi2.metric("Mes de Cierre", f"{NOMBRES_MESES[resultado.mes_objetivo - 1]} ({resultado.mes_objetivo})")
        ...
        st.caption(f"**Secuencia de la Ventana Temporal:** {' → '.join(resultado.ventana_temporal)}")
        ...
        with tab_graficos:
            ...
            if len(df_plot) > 0:
                ax.bar(df_plot.index.astype(str), df_plot["Total"], color="#D9381E", edgecolor="black", alpha=0.85)
                ax.set_title(f"Años Análogos Principales (Año Objetivo: {resultado.year_objetivo}, Mes: {NOMBRES_MESES[resultado.mes_objetivo - 1]})", fontsize=13, pad=10)
            ...
            if len(resultado.ranking) > 0 and resultado.ranking[0][1] > 0:
                ...
                st.markdown(f"### Comparación de Trayectoria Temporal: Objetivo ({resultado.year_objetivo}) vs Análogo #{1} ({top_year})")
                
                fig_comp, axes = plt.subplots(len(resultado.indices_evaluados), 1, figsize=(12, 3.5 * len(resultado.indices_evaluados)), sharex=True)
                ...
                for ax_i, osc in zip(axes, resultado.indices_evaluados):
                    ...
                    v_obj = extraer_ventana(df_osc, resultado.year_objetivo, resultado.mes_objetivo, longitud_ventana=resultado.longitud_ventana)
                    v_cand = extraer_ventana(df_osc, top_year, resultado.mes_objetivo, longitud_ventana=resultado.longitud_ventana)
                    
                    x_labels = resultado.ventana_temporal
                    ax_i.plot(x_labels, v_obj, marker="o", linewidth=2.2, color="#1f77b4", label=f"Objetivo ({resultado.year_objetivo})")
                    ax_i.plot(x_labels, v_cand, marker="s", linewidth=2.0, linestyle="--", color="#d62728", label=f"Análogo {top_year}")
                    ax_i.set_title(f"Índice: {osc} ({CATALOGO.get(osc, {}).get('name', osc)})", fontsize=11)
                    ax_i.set_ylabel(f"Valor ({CATALOGO.get(osc, {}).get('units', 'u')})", fontsize=10)
...
# ==============================================================================
    ...
    with col_card1:
        ...
        st.markdown(f"**Variable Climatológica:** {meta.get('variable', 'No especificada')}")
        st.markdown(f"**Región de Referencia:** {meta.get('region', 'No especificada')}")
    ...
    with col_card2:
        ...
        st.markdown(f"**Umbral Metodológico Pearson ($r_{{\\text{{umbral}}}}$):** `{r_def:.2f}`")
        st.markdown(f"**Umbral Metodológico MAD ($\\text{{MAD}}_{{\\text{{umbral}}}}$):** `{mad_def:.2f}`")
        st.markdown(f"**Fuente Operacional de Datos:** {meta.get('institution', 'No especificada')}")
        if meta.get("url"):
            st.markdown(f"**Enlace de Descarga:** [{meta.get('url')}]({meta.get('url')})")
        if meta.get("reference"):
            st.markdown(f"**Referencia Científica:** *{meta.get('reference')}*")
        if meta.get("doi"):
            st.markdown(f"**DOI:** [{meta.get('doi')}](https://doi.org/{meta.get('doi')})")

    # Descripción climatológica detallada
    if meta.get("description"):
        st.info(f"**Descripción y Relevancia Climatológica:** {meta.get('description')}")

    st.divider()

    # Visualización de la Serie Temporal
    st.subheader(f"📈 Evolución Temporal del Índice {indice_sel}")
    if indice_sel in oscilaciones_disponibles and oscilaciones_disponibles[indice_sel] is not None:
        df_ind = oscilaciones_disponibles[indice_sel].copy()
        
        # Filtro de rango de años
        filtro_tiempo = st.radio(
            "Rango Temporal:",
            ["Todo el registro", "Últimos 10 años", "Últimos 5 años", "Rango personalizado"],
            horizontal=True
        )
        ...
        ax_ts.set_title(f"Serie Mensual: {meta.get('name', indice_sel)} ({y_start}–{y_end})", fontsize=12)
        ...
        st.error(f"No se encontraron datos locales cargados para el índice {indice_sel}.")
...
# ==============================================================================
    ...
    col_stat2.metric("Cobertura Temporal Promedio", f"{int(df_salud['Años Registrados'].mean()) if disponibles > 0 else 0} años")
    col_stat3.metric("Último Año con Registros", f"{df_salud['Último Año'].max() if disponibles > 0 else '-'}")
    ...
    if st.button("↻ Actualizar índices", type="primary", help="Descargar y verificar las últimas series desde fuentes oficiales"):
        ...
        res_update = verificar_y_descargar_datos(DIRECTORIO_ACTUAL, force_update=True, progress_callback=update_progress)
        prog_bar.empty()
        status_text.empty()
        ...
        todos_ok = all(v["status"] in ["OK", "Sin cambios"] for v in res_update.values())
        if todos_ok:
            st.success("✓ Índices actualizados correctamente.")
        else:
            st.warning("⚠ Algunos índices no pudieron actualizarse (se conservaron las copias locales previas).")
        
        # Resumen de actualización
        res_rows = []
        for k, v in res_update.items():
            res_rows.append({"Índice": k, "Estado": v["status"], "Detalle": v["mensaje"]})
        st.dataframe(pd.DataFrame(res_rows), use_container_width=True)

    st.subheader("📋 Inventario Detallado de Fuentes")
    st.dataframe(
        df_salud[["Código", "Nombre", "Estado", "Primer Año", "Último Año", "Último Mes", "Años Registrados", "Institución"]],
        use_container_width=True,
        height=450
    )
...
# ==============================================================================
    ...
    with col_btn2:
        if st.button("🔄 Restaurar Valores Metodológicos Predeterminados", use_container_width=True):
            st.session_state["umbrales_usuario"] = obtener_umbrales_metodologicos()
    ...
    for i, cod in enumerate(indices_lista):
        with cols_th[i % 3]:
            ...
            r_curr, mad_curr = st.session_state["umbrales_usuario"].get(cod, (r_metod, mad_metod))
            ...
            es_default = (abs(r_val - r_metod) < 1e-4) and (abs(mad_val - mad_metod) < 1e-4)
            if not es_default:
